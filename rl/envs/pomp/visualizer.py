...
class PlanVisualizationProgram(GLProgram):
    ...
    """
    def __init__(self,problem,planner,plannerFilePrefix, use_objects=False):
    ...
    def idlefunc (self):
        if self.save_movie:
            if self.painted:
                ...
                self.G = self.planner.getRoadmap(use_objects=self.use_objects)
                self.refresh()
                self.painted = False
    ...
    def display(self):
        ...
        if self.G:            #draw graph
            ...
            if self.use_objects:
                ...
                glPointSize(3.0)
                V_states = [vertex.state.tolist() for vertex in V]
                # for vertex in V: 
                #     self.problem.visualizer.drawVerticesGL([vertex.state.tolist()])
                self.problem.visualizer.drawVerticesGL(V_states)
                for (i,j,e) in E:
                    # if V[j].pessimistic_value > 1: 
                    # if V[j].V > 1 and False: 
                    if V[j].V > 1: 
                    # if V[j].kernel_value > 5: 
                        glLineWidth(3.0)
                        glColor3f(0,0.75,0)
                    else:
                        glLineWidth(1.0)
                    ...
                    interpolator = self.problem.space.interpolator(V_states[i],e)
                    # import ipdb
                    # ipdb.set_trace()
                    if np.linalg.norm(V[i].state - V[j].state) > 0.001:
...
def run_gym_visualizer(env, planner):
    use_objects = False
    use_objects = True
    mkdir_p("data")
    problem = env.problem
    program = PlanVisualizationProgram(problem,planner,os.path.join("data", ""), use_objects=use_objects)
        # allplanners.filename[plannerType]))
    program.width = program.height = 640
    program.run()
