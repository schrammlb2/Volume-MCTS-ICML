...
from .rrtstarplanner import RRTStar, StableSparseRRT, StableSparseRRTStar
from .rrtstarplanner_2 import BidirectionalPlanner
from .mctsplanner import MCTSPlanner, VolumeMCTS, KDVolumeMCTS
...
all_planners += ['bidirectional-rrt*']
all_planners += ['mcts']
all_planners += ['volume_mcts']
all_planners += ['kdvolume_mcts']
...
filename = {'ao-rrt':'ao_rrt',
            ...
            'bidirectional-rrt*': 'bidirectional_rrtstar',
            'mcts': 'mcts',
            'volume_mcts': 'volume_mcts',
            'kdvolume_mcts': 'kdvolume_mcts'}
...
def makePlanner(type,space,start,goal,
    ...
    if type == 'rrt*':
        ...
        if isinstance(controlSpace,ControlSpaceAdaptor):
            planner.setControlSelector(KinematicControlSelector(controlSpace,controlSpace.nextStateSamplingRange))
    elif type == 'bidirectional-rrt*':
        planner = BidirectionalPlanner(space,metric,checker,**params)
        # planner = BidirectionalPlanner(RRTStar)
        # planner.initialize(space,metric,checker,**params)
    elif type == 'mcts':
        planner = MCTSPlanner(controlSpace,objective,metric,checker)#,**params)
        #set direct steering functions for kinematic spaces 
        if isinstance(controlSpace,ControlSpaceAdaptor):
            planner.setControlSelector(KinematicControlSelector(controlSpace,controlSpace.nextStateSamplingRange))
    elif type == 'volume_mcts':
        planner = VolumeMCTS(controlSpace,objective,metric,checker)#,**params)
        if isinstance(controlSpace,ControlSpaceAdaptor):
            planner.setControlSelector(KinematicControlSelector(controlSpace,controlSpace.nextStateSamplingRange))
    elif type == 'kdvolume_mcts':
        planner = KDVolumeMCTS(controlSpace,objective,metric,checker)#,**params)
        #set direct steering functions for kinematic spaces 
        if isinstance(controlSpace,ControlSpaceAdaptor):
            planner.setControlSelector(KinematicControlSelector(controlSpace,controlSpace.nextStateSamplingRange))
    else:
