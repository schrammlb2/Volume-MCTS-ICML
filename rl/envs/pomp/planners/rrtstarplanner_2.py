...
class RRTStarMod(TreePlanner):
    ...
    def __init__(self,cspace,metric,edgeChecker,
        ...
        # self.nearestNeighbors = KDTree(self.metric)
    ...
    def ctg_upper_bound(self, x):
        ...
        return min(ctg(x, self.goal.c), self.nearestNeighbors.ctg_upper_bound(x))
    ...
    #     return min(ctg(x, self.goal.c), min_dist)
    ...
    def expand(self):
        ...
        cost = nnear.c + edge.length()
        self.set_cost(nnew, cost)
    ...
    def set_cost(self, n, cost):
        cost_bound = cost + self.ctg_upper_bound(n.x)
        # cost_bound = cost + self.nearestNeighbors.ctg_upper_bound(n.x)
        n.set_cost(cost)
        bounded_pts = n.set_cost_bound(cost_bound)
        self.nearestNeighbors.set_new_cost_bound(bounded_pts, cost_bound)
...
class StableSparseRRT(TreePlanner):
    ...
    #     if self.root == None:
    #         self.root = Node([(point,data)], (self.lo, self.hi))
    #         self.maxDepth = 0
    #         self.numNodes = 1
...
class BidirectionalPlanner(RRTStarMod):
    # def __init__(self, *args, **kwargs):
        # self.forward_tree = RRTStar(*args, **kwargs)
    def __init__(self, space,metric,checker,**params):
        self.space = space
        self.metric = metric
        # import ipdb
        # ipdb.set_trace()
        self.forward_tree = RRTStarMod(space,metric,checker,**params)
        self.backward_tree = RRTStarMod(space,metric,checker,**params)
        self.forward_tree.configurationSampler = None
        self.forward_tree.nearestNeighbors = None
        self.backward_tree.configurationSampler = None
        self.backward_tree.nearestNeighbors = None
        # self.backward_tree = RRTStar(*args, **kwargs)
        self.tree_policy = None

    def destroy(self):
        self.forward_tree.destroy()
        self.backward_tree.destroy()

    def reset(self):
        self.forward_tree.reset()
    #     """Add a point to the KD tree (O(log(n)) running time)"""
    #     if self.root == None:
    #         self.root = Node([(point,data)], (self.lo, self.hi))
    #         self.maxDepth = 0
    ...
    #     return return_value
        self.backward_tree.reset()

    def _setSampler(self):
        self.forward_tree_policy = KDTreePolicy(distanceMetric=self.metric,cspace=self.space, goal=self.goal)
        # import ipdb; ipdb.set_trace()
        self.forward_tree.configurationSampler = self.forward_tree_policy
        self.forward_tree.nearestNeighbors = self.forward_tree_policy

        self.backward_tree_policy = KDTreePolicy(distanceMetric=self.metric,cspace=self.space, goal=self.start)
        self.backward_tree.configurationSampler = self.backward_tree_policy
        self.backward_tree.nearestNeighbors = self.backward_tree_policy

        self.forward_tree_policy.set_other_kdtree(self.backward_tree_policy)
        self.backward_tree_policy.set_other_kdtree(self.forward_tree_policy)
        
        self.root = self.forward_tree.root
        self.stats = self.forward_tree.stats


        # self.backward_tree.configurationSampler = self.tree_policy
        # self.backward_tree.nearestNeighbors = self.tree_policy
    ...
    def setBoundaryConditions(self,x0,goal):
        ...
        self.goal = goal.c
        self._setSampler()
        self.forward_tree.setBoundaryConditions(x0,goal)
        start_set = NeighborhoodSubset(self.space,x0,goal.r)
        # For SST and other kinodynamic algs, use self.space.configurationSpace() instead
        # start_set = NeighborhoodSubset(self.space.configurationSpace(),x0,goal.r)
        self.backward_tree.setBoundaryConditions(goal.c, start_set)

        # import ipdb
        # ipdb.set_trace()
        # self.backward_tree.setBoundaryConditions(x0,goal)

    def setConfigurationSampler(self,sampler):
        self.forward_tree.setConfigurationSampler(sampler)
        self.backward_tree.setConfigurationSampler(sampler)

    def planMore(self,iters):
        assert self.forward_tree_policy is not None
        assert self.backward_tree_policy is not None
        self.forward_tree.planMore(iters)
        self.backward_tree.planMore(iters)

    def getBestPathCost(self):
        return self.forward_tree.getBestPathCost()
    # def expand(self):
    ...
    def getPath(self,n=None):
        if n == None:
            return self.forward_tree.bestPath
        return TreePlanner.getPath(self,n)

    def getRoadmap(self):
        # return self.forward_tree.getRoadmap()
        return merge_graph(self.forward_tree.getRoadmap(), self.backward_tree.getRoadmap())
...
def merge_graph(g1, g2):
    ...
    v2, e2 = g2
    offset = len(v1)
    new_e2 = [(e[0]+offset, e[1] + offset, e[2]) for e in e2]

    return (v1 + v2, e1 + new_e2)