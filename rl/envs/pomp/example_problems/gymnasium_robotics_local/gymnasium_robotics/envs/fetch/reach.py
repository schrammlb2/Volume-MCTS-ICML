...
from . import MujocoFetchEnv, MujocoPyFetchEnv#, RotationMujocoFetchEnv

import numpy as np
# Ensure we get the path separator correct on windows
MODEL_XML_PATH = os.path.join("fetch", "reach.xml")
# MODEL_XML_PATH_2 = os.path.join("fetch", "reach.xml")
...
# class RotationMujocoFetchReachEnv(RotationMujocoFetchEnv, EzPickle):
class RotationMujocoFetchReachEnv(MujocoFetchEnv, EzPickle):
    def __init__(self, reward_type: str = "sparse", **kwargs):
        initial_qpos = {
            "robot0:slide0": 0.4049,
            "robot0:slide1": 0.48,
        ...
        EzPickle.__init__(self, reward_type=reward_type, **kwargs)
        self.saved_fullpath = self.fullpath
        if self.saved_fullpath[-4:] == ".xml":
            i = 0
            self.fullpath = self.saved_fullpath[:-4] + f"_{i}" + ".xml"
    ...
    def reset(self, **kwargs):
        ...
        if not os.path.exists(self.fullpath):
            raise OSError(f"File {self.fullpath} does not exist")
        self._initialize_simulation()
        return MujocoFetchEnv.reset(self, **kwargs)


# class RandomFetchReachEnv(MujocoFetchEnv, EzPickle):
#     def __init__(self, reward_type: str = "sparse", **kwargs):
#         initial_qpos = {
#             "robot0:slide0": 0.4049,
#             "robot0:slide1": 0.48,
...
#     def reset(self, **kwargs):
#         if self.saved_fullpath[-4:] == ".xml":
#             self.fullpath = self.saved_fullpath[:-4] + f"_{np.random.randint(3)}" + ".xml"
#         if not os.path.exists(self.fullpath):
#             raise OSError(f"File {self.fullpath} does not exist")
#         self._initialize_simulation()
#         return MujocoFetchEnv.reset(self, **kwargs)
