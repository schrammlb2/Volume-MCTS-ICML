...
def get_base_fetch_env(RobotEnvClass: Union[MujocoPyRobotEnv, MujocoRobotEnv]):
    ...
    class BaseFetchEnv(RobotEnvClass):
        ...
        def get_state(self):
            ...
            return np.concatenate([np.array([time]), qpos, qvel])

        def set_state(self, env, vec):
            env.data.time = vec[0]
            q_len = len(self.data.qpos[:])
            env.data.qpos[:] = vec[1:q_len+1]
            env.data.qvel[:] = vec[1+q_len:]
...
class MujocoFetchEnv(get_base_fetch_env(MujocoRobotEnv)):
    def __init__(self, default_camera_config: dict = DEFAULT_CAMERA_CONFIG, rotation=False, **kwargs):        
        super().__init__(default_camera_config=default_camera_config, rotation=rotation, **kwargs)
