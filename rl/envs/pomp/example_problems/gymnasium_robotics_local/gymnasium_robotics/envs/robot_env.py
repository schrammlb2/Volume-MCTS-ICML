...
try:
    ...
    from ..utils import mujoco_py_utils
...
class BaseRobotEnv(GoalEnv):
    ...
    def __init__(
        ...
        self.preinitial_qpos = initial_qpos
        ...
        self.observation_space = spaces.Dict(
            dict(
                ...
                state=spaces.Box(
                    -np.inf, np.inf, shape=obs["state"].shape, dtype="float64"
                ),
            )
        )
...
class MujocoRobotEnv(BaseRobotEnv):
    ...
    def _initialize_simulation(self):
        ...
        self._env_setup(initial_qpos=self.preinitial_qpos)
