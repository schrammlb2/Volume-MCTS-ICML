...
class HOOTTrainer(Trainer):
    def __init__(self, cfg, Env, unwrapped_env):
        ...
        model = self.her_agent.get_policy_interface()
        model.action_bound = cfg.policy.action_bound
        model.action_dim   = cfg.policy.action_dim

        # her_agent.learn()

        self.agent.set_network(model)
    ...
    def run(self, Env, epoch):
        ...
        for t in range(self.cfg.max_episode_length):
            ...
            #     terminal_reward = step_reward*(cfg.max_episode_length - 1 - t)
            #     R += step_reward
