...
from HER.rl_modules.models import PolicyInterfaceV
...
class ddpg_agent:
    ...
    def get_policy_interface(self):    
        return PolicyInterfaceV(self.actor_network, self.critic_network, 
    ...
    def _update_network(self, future_p=None):
        ...
        w_e = torch.clamp(normed_vol*torch.exp(e_v.detach()), 0, 10)
    ...
    def _eval_agent(self, final=False):
        ...
        for i in range(self.args.n_test_rollouts*test_num_multiplier):
            ...
            with torch.no_grad():
                ...
                # value = self.critic_network(inputs_norm_tensor).mean().numpy().squeeze()
                # total_value += value
