...
def run_agent(cfg: DictConfig):
    ...
    logger = Logger(["maze_size", "R"], filename=name + "_with_training", env_name=env_base)
    R_max = -np.inf
    best_actions = None
    actions_list = []
    ...
    for maze_size in pbar:
        ...
        for training_run in range(3):
            ...
            cfg.mcts.n_rollouts = global_config.n_rollouts_per_step*global_config.max_episode_length
            ...
            # for _ in range(1):
                ...
                agent.epoch = maze_size
                action_buffer = []
                action_buffer, dict_list = agent.act(Env=Env)
                t_total += (
