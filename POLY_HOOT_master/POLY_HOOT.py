...
import copy
import gym
from POLY_HOOT_master.SnapshotENV import SnapshotEnv
import pickle
import os
import numpy as np
from POLY_HOOT_master.POLY_HOO import POLY_HOO
import random
...
class Node:
	def __init__(self, snapshot, obs, is_done, parent, depth, dim, env):
		self.parent = parent
		...
		rho = 2**(-2 / dim)
		nu = 4 * dim
		self.hoo = POLY_HOO(dim=dim, nu=nu, rho=rho, min_value=-1, max_value=1, lim_depth=HOO_LIMIT_DEPTH, alpha=alphas[depth], xi=xis[depth], eta=etas[depth])
	...
	def selection(self, depth):
		...
		if tuple(action) in self.children:
			...
			snapshot, obs, immediate_reward, is_done, _ = self.env.get_result(self.snapshot, action)
			child = Node(snapshot, obs, is_done, self, depth + 1, self.dim, self.env)
